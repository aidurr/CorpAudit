...
impl HistoryManager {
    ...
    pub fn save_scan(&self, report: &AuditReport) -> Result<ScanHistory> {
        ...
        let content =
            serde_json::to_string_pretty(&history).context("Failed to serialize scan history")?;
    ...
    pub fn load_timeline(&self, days: u32) -> Result<Vec<(String, usize, usize, usize)>> {
        let histories = self.load_history(days)?;

        let timeline = histories
            .into_iter()
            .map(|h| {
                (
                    h.timestamp,
    ...
    pub fn analyze_trends(&self, days: u32) -> Result<TrendAnalysis> {
        ...
        if histories.is_empty() {
            return Ok(TrendAnalysis {
                ...
                changes: Vec::new(),
                recommendations: vec![
                    "No historical data available. Run multiple scans to generate trend data."
                        .to_string(),
                ],
            });
        }
        ...
        let privacy_trend = match &telemetry_trend {
            TrendDirection::Increasing { percentage } => TrendDirection::Decreasing {
                percentage: *percentage,
            },
            TrendDirection::Decreasing { percentage } => TrendDirection::Increasing {
                percentage: *percentage,
            },
            other => other.clone(),
        };
        ...
        let recommendations =
            self.generate_trend_recommendations(&histories, &telemetry_trend, &bloat_trend)?;
    ...
    pub fn load_scan_by_id(&self, scan_id: &str) -> Result<Option<ScanHistory>> {
        ...
        let history: ScanHistory = serde_json::from_str(&content).context(format!(
    ...
    fn extract_history_data(&self, report: &AuditReport) -> ScanHistory {
        ...
        let total_data_received_bytes = report.telemetry.as_ref().map_or(0, |t| {
            t.findings
                .iter()
                .map(|f| f.data_received.unwrap_or(0))
                .sum()
        });
    ...
    fn calculate_trend(values: &[usize]) -> TrendDirection {
        ...
        let first_half: Vec<_> = values[..values.len() / 2]
            .iter()
            .map(|v| *v as f64)
            .collect();
        let second_half: Vec<_> = values[values.len() / 2..]
            .iter()
            .map(|v| *v as f64)
            .collect();
        ...
        if percentage_change.abs() < 10.0 {
            ...
            TrendDirection::Increasing {
                percentage: percentage_change.abs(),
            }
        } else {
    ...
    fn generate_trend_recommendations(
        ...
        if histories.len() >= 3 {
            let recent_telemetry: Vec<_> = histories
                .iter()
                .take(3)
                .map(|h| h.telemetry_count)
                .collect();
        ...
        if recommendations.is_empty() {
            recommendations
                .push("No specific recommendations. Continue monitoring for trends.".to_string());
        }
